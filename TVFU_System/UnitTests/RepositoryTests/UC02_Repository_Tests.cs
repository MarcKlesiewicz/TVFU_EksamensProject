...
{
    [TestClass]
    public class UC02_Repository_Tests
    {
        ProductRepo repo;
        ...
        {
            //Arrange
            var expectedObject = new ProductEventArgs(testList[2]);
            expectedObject.Description = "test5UpdateChanged";

            var updateArgs = new ProductEventArgs(expectedObject);
            ...
            testList[2].Description = "test5UpdateChanged";

            repo.Update(updateArgs);

            //Assert
            List<Product> actualList = (repo.GetByProductNumber(1901, 1903) as List<Product>);
            string actualResult = String.Join("\n", actualList);
            string expectedResult = testList[2].ToString();
